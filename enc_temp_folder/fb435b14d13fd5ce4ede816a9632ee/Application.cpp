...
void application::process_framebuffer_size_callback(application& app, int width, int height) {
	...
	if (app.rml_render_interface) {
		app.rml_render_interface->set_viewport(width, height);
	}
}
...
inline application& application::get_application_ref(GLFWwindow* window) {
	...
	if (!user_pointer) {
		throw std::runtime_error("Unable to retrieve window user pointer.");
	}
...
void application::init(const application_build_params& appbp) {
	...
	rml_render_interface = std::make_unique<diligent_rml_render_interface>(*this, viewport_width, viewport_height);
...
diligent_rml_render_interface::diligent_rml_render_interface(application& app_in, int init_viewport_width, int init_viewport_height) : 
	app(app_in),
	viewport_width(init_viewport_width),
	viewport_height(init_viewport_height)
{
...
void diligent_rml_render_interface::create_pipeline_states()
{
	...
	RefCntAutoPtr<IShader> vertex_shader;
	{
		...
		shader_create_info.Desc.Name = "Rml vertex shader";
		shader_create_info.Source = rml_main_vertex_shader;
		app.device->CreateShader(shader_create_info, &vertex_shader);
	}
	...
	RefCntAutoPtr<IShader> color_pixel_shader;
	{
		...
		shader_create_info.EntryPoint = "main";
		shader_create_info.Desc.Name = "Rml color shader";
		shader_create_info.Source = rml_color_pixel_shader;
		app.device->CreateShader(shader_create_info, &color_pixel_shader);
	}

	std::array layout_elems = {
		LayoutElement{0, 0, 2, VT_FLOAT32, False, offsetof(Rml::Vertex, position),  sizeof(Rml::Vertex)},
		LayoutElement{1, 0, 4, VT_UINT8  , True,  offsetof(Rml::Vertex, colour),    sizeof(Rml::Vertex)},
		LayoutElement{2, 0, 2, VT_FLOAT32, False, offsetof(Rml::Vertex, tex_coord), sizeof(Rml::Vertex)}
	};

	pipeline_state_create_info.GraphicsPipeline.InputLayout.LayoutElements = layout_elems.data();
	pipeline_state_create_info.GraphicsPipeline.InputLayout.NumElements = static_cast<unsigned int> (layout_elems.size());
...

	{
		...
		buffer_desc.Name = "VS constant buffer";
		buffer_desc.Size = sizeof(constants);
		app.device->CreateBuffer(buffer_desc, nullptr, &rml_constants_buffer);
	}
	...
	{
		ImmutableSamplerDesc {SHADER_TYPE_PIXEL, "_texture_sampler", linear_clamp_desc}
	};
	
	pipeline_state_create_info.PSODesc.ResourceLayout.ImmutableSamplers = immutable_samplers.data();
	pipeline_state_create_info.PSODesc.ResourceLayout.NumImmutableSamplers = static_cast<unsigned int> (immutable_samplers.size());
...
{
	std::span vertices_span(vertices, num_vertices);
	std::span indices_span(indices, num_indices);
	
	render_span_geometry(vertices_span, indices_span, translation, texture);
}

void diligent_rml_render_interface::render_span_geometry(const std::span<Rml::Vertex>& vertices_span, const std::span<int>& indices_span, const Rml::Vector2f& translation, Rml::TextureHandle& texture)
{
	// Memory requirements
	...
	vertex_buffer_descriptor.Size = vertices_span.size() * sizeof(Rml::Vertex);
	BufferData vertex_buffer_data;
	vertex_buffer_data.pData = vertices_span.data();
	vertex_buffer_data.DataSize = vertices_span.size() * sizeof(Rml::Vertex);
	...
	index_buffer_desc.Size = indices_span.size() * sizeof(int);
	BufferData index_buffer_data;
	index_buffer_data.pData = indices_span.data();
	index_buffer_data.DataSize = indices_span.size() * sizeof(int);
	...
	*cb_constants = c;
	...
	draw_attributes.NumIndices = static_cast<unsigned int> (indices_span.size ());
...
bool diligent_rml_render_interface::GenerateTexture(Rml::TextureHandle& texture_handle, const Rml::byte* source, const Rml::Vector2i& source_dimensions)
{
	...
	RefCntAutoPtr<ITexture> tex;
	{
		...
		std::array mipdata = {
			TextureSubResData{source, 4u * source_dimensions.x }
		};
...
void diligent_rml_render_interface::begin_frame()
{
	...
	projection = Rml::Matrix4f::ProjectOrtho(0.0f, static_cast<float> (viewport_width),
		static_cast<float> (viewport_height), 0.0f, //swapping bottom and top inputs puts (0,0) in the top left corner instead of the bottom left corner
		-10000.0f, 10000.0f);
